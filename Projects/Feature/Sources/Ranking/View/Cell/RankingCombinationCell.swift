...
final class RankingCombinationCell: BaseCollectionViewCell<RankingCombination> {
    ...
    private lazy var imageStackView = UIStackView().then {
        $0.axis = .horizontal
        $0.spacing = -16
        $0.addSubviews([drinkImageView, snackImageView])
    }
    ...
    override func bind(_ model: RankingCombination) {
        super.bind(model)
        drinkNameLabel.text = model.drink
        rankLabel.text = model.snack
    }
}
